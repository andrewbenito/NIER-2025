...
df2019 <- df1 %>%
  ...
  group_by(ISO) %>%
  mutate(
    gdppc1960 = `Per Capita Income`[year == 1960],
    gdppc2000 = `Per Capita Income`[year == 2000],
    g19602019 = 100 *
      ((`Per Capita Income`[year == 2019] /
        `Per Capita Income`[year == 1960])^(1 / (2019 - 1960)) -
        1),
    g20002019 = 100 *
      ((`Per Capita Income`[year == 2019] /
        `Per Capita Income`[year == 2000])^(1 / (2019 - 2000)) -
        1)
  ) %>%
...
df3 <- df1 %>%
  group_by(year) %>%
  mutate(
    dlyhMin = min(`Output per Hour Worked growth`, na.rm = TRUE),
...
df20 <- df1 %>%
  ...
  mutate(
    dyl20y = rollmean(
      x = `Output per Employed Person growth`,
      20,
      fill = NA,
      align = "right"
    ),
...
byISO <- df20 %>%
  ...
  summarise(across(
    c(
      'Output per Employed Person growth',
      'Employment growth',
      'Population growth',
...
byISOTime <- df20 %>%
  ...
  mutate(
    ylslow = ifelse(
      decade == '2010-19',
      `Output per Employed Person growth` -
        lag(`Output per Employed Person growth`, 2),
      NA
    ),
    dPopG = ifelse(
      decade == '2010-19',
      `Population growth` - lag(`Population growth`, 2),
      NA
    )
  )
...
byYear <- df20 %>%
  ...
  nest(data = -year) %>%
  mutate(
    fit = map(data, ~ lm(dyl20y ~ dPop20y + ylinitial, data = .x)),
    tidied = map(fit, tidy),
