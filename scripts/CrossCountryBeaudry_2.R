...
# ABenito Nov 2023. Used for Paper revision

# Packages ----
rm(list=ls())
x = c('tidyverse', 'readxl', 'broom', 'modelr', 'PooledMeanGroup', 'ggtext', 'ggrepel', 'ggsci',
...
# Nested list-columns by Country
#===============================
byCountry <- df1 %>%
  filter(ylinitial!="NA") %>%
  nest(data = -ISO) %>%
...
byCountry |> unnest(tidied) |> filter(ISO=="GBR")
